...
public class CPStatesProvider extends BlockStateProvider {
	...
	protected void registerStatesAndModels() {
		horizontalAxisBlock(CPBlocks.stew_pot,bmf("cistern"));
		
		horizontalMultipart(horizontalMultipart(this.getMultipartBuilder(CPBlocks.stove1),bmf("kitchen_stove_i"))
		...
		horizontalMultipart(horizontalMultipart(this.getMultipartBuilder(CPBlocks.stove2),bmf("kitchen_stove_ii"))
		.part().modelFile(bmf("kitchen_stove_ash")).addModel().condition(KitchenStove.LIT,false).condition(KitchenStove.ASH,true).end()
		.part().modelFile(bmf("kitchen_stove_hot_ash")).addModel().condition(KitchenStove.LIT,true).end()
		,bmf("kitchen_stove_charcoal"),i->i.condition(KitchenStove.FUELED,1));
		itemModel(CPBlocks.stove1,bmf("kitchen_stove_i"));
		itemModel(CPBlocks.stove2,bmf("kitchen_stove_ii"));
		itemModel(CPBlocks.stew_pot,bmf("cistern"));
	}
