...
public class DoliumRestingCategory implements IRecipeCategory<DoliumRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, DoliumRecipe recipe, IFocusGroup focuses) {
    	if(recipe.items.size()>0) {
    		builder.addSlot(type(recipe.items.get(0)),4, 6).addIngredients(VanillaTypes.ITEM,unpack(recipe.items.get(0))).addTooltipCallback(cb(recipe.items.get(0)));
    		if(recipe.items.size()>1) {
        		builder.addSlot(type(recipe.items.get(1)),4, 24).addIngredients(VanillaTypes.ITEM,unpack(recipe.items.get(1))).addTooltipCallback(cb(recipe.items.get(1)));
        		if(recipe.items.size()>2) {
            		builder.addSlot(type(recipe.items.get(2)),4, 42).addIngredients(VanillaTypes.ITEM,unpack(recipe.items.get(2))).addTooltipCallback(cb(recipe.items.get(2)));
        		}
    		}
    	}
    	...
    	if(!(recipe.fluid==Fluids.EMPTY))
    		builder.addSlot(RecipeIngredientRole.INPUT,26, 9).addIngredient(VanillaTypes.FLUID,new FluidStack(recipe.fluid,recipe.amount)).setFluidRenderer(1250, false,16,46).addTooltipCallback(new BaseCallback(recipe.base,recipe.density));
