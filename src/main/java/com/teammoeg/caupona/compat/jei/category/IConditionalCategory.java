...
public abstract class IConditionalCategory<T extends IConditionalRecipe&Recipe<?>> implements CPCategory<RecipeHolder<T>> {
	...
	public static IDrawable grid(IGuiHelper helper,int x,int y,int dx,int dy,int w,int h) {
		return helper.createDrawable(Identifier.fromNamespaceAndPath(CPMain.MODID,"textures/gui/recipes/elements/recipe_page_elements.png"), x*47+dx, y*53+dy, w, h);
	}
	public abstract IDrawable getHeadings();
	public abstract void drawCustom(RecipeHolder<T> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor stack, double mouseX,double mouseY);
	@Override
	public void draw(RecipeHolder<T> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor stack, double mouseX,double mouseY) {
	...
	public boolean drawConditionList(GuiGraphicsExtractor stack,List<IngredientCondition> conditions,int offX,int offY) {
