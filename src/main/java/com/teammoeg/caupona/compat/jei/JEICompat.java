...
public class JEICompat implements IModPlugin {
	...
	public Identifier getPluginUid() {
		return Identifier.fromNamespaceAndPath(CPMain.MODID, "jei_plugin");
	}

	@Override
	public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
		registration.addCraftingStation(BrazierCategory.TYPE, new ItemStack(CPItems.pbrazier.get()));
		registration.addCraftingStation(PotCategory.TYPE, new ItemStack(CPBlocks.STEW_POT.get()));
		registration.addCraftingStation(BoilingCategory.TYPE, new ItemStack(CPBlocks.STEW_POT.get()));
		registration.addCraftingStation(PotRestingCategory.TYPE, new ItemStack(CPBlocks.STEW_POT.get()));
		registration.addCraftingStation(StewCookingCategory.TYPE, new ItemStack(CPBlocks.STEW_POT.get()));
		for (Block bl : CPBlocks.dolium) {
			registration.addCraftingStation(DoliumRestingCategory.TYPE,new ItemStack(bl));
			registration.addCraftingStation(PotRestingCategory.TYPE,new ItemStack(bl));
		}
		registration.addCraftingStation(FryingCategory.TYPE, new ItemStack(CPBlocks.STONE_PAN.get()));
		registration.addCraftingStation(FryingCategory.TYPE, new ItemStack(CPBlocks.COPPER_PAN.get()));
		registration.addCraftingStation(FryingCategory.TYPE, new ItemStack(CPBlocks.IRON_PAN.get()));
	}
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		registration.addItemStackInfo(new ItemStack(CPItems.LEADEN_WALNUT.get()),Utils.translate("gui.jei.caupona.info.leaden_walnut"));
	}
	...
	public void registerGuiHandlers(IGuiHandlerRegistration registry) {
		...
		registry.addGuiContainerHandler(DoliumScreen.class,  new GuiTankHandler<DoliumScreen>(ingredientManager).addTank(80, 27, 16, 46, t->FluidUtil.getStack(t.getBlockEntity().getTank(), 0)));
		...
		registry.addGuiContainerHandler(StewPotScreen.class,  new GuiTankHandler<StewPotScreen>(ingredientManager).addTank(105, 20, 16, 46, t->FluidUtil.getStack(t.getBlockEntity().getTank(), 0)));
		registry.addRecipeClickArea(KitchenStoveScreen.class, 61, 0, 54, 28, RecipeTypes.SMELTING_FUEL);
