...
public abstract class FileGenerator implements DataProvider {
	...
	protected class FileStorage implements BiConsumer<Path, byte[]>, Consumer<FileOutput> {
		...
		public void accept(ResourceLocation rl, byte[] s) {
			Path p = output.getOutputFolder()
					.resolve(type.getDirectory() + "/" + rl.getNamespace() + "/" + rl.getPath());
			this.accept(p, s);
		}
	...
	public FileGenerator(PackType pt, DataGenerator output, ExistingFileHelper helper, String name) {
		super();
		this.type = pt;
		this.output = output;
		this.helper = helper;
	...
	static void saveFile(CachedOutput cache, byte[] data, Path path) {
